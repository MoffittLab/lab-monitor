...
def ingest_reports():
    ...
    if not validate_token(token):
        _logger.warning(f"Invalid token received")
    ...
    for report in reports:
        ...
        if not nas_name:
            _logger.warning("Report missing nas_name, skipping")
            continue
        
        if append_report_to_log(report, nas_name):
            success_count += 1
            _logger.info(f"Stored report from {nas_name}")
        else:
...
def get_all_usage():
    """
    Get latest usage for all NAS systems.
...
def get_nas_latest(nas_name):
    """
    Get latest usage for a specific NAS.
...
def get_nas_history(nas_name):
    """
    Get usage history for a NAS within a date range.
